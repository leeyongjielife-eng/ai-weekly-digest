...
- [x] `F-018C-2` GitHub Actions 自动更新与部署
...
- 当前公开站点恢复为 69 篇文章、4 个日期页、8 个分类页和 69 个详情页；线上旧的 `2026-09-13` 地址已确认返回 404；
- 2026-09-14 首次本地定时运行因系统注入的 `127.0.0.1:7897` 代理导致 Gmail OAuth 刷新失败，已让定时包装默认清理系统代理变量；修复后真实重跑成功，新增 15 篇，站点达到 84 篇、5 个日期页和 84 个详情页。
